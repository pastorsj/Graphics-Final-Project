...
void Wall::init(string objName, int texNum, int xCell, int yCell, bool rotate, float yOffset)
{
	immune = 0;
	...
	for (int i = 1; i < this->mazeGen->getXSize(); ++i)
	{
		for (int j = 1; j < this->mazeGen->getYSize(); ++j)
		{
			glm::mat4 buildTrans = glm::translate(glm::mat4(1.0f), glm::vec3(i, 0, j));
			if (this->mazeGen->getCell(i, j).up)
			{
				postTrans.push_back(buildTrans);
			}
			if (this->mazeGen->getCell(i, j).left)
			{
